...
public class Herdspawner extends Spawner {
	...
	public Herdspawner(Location loc1, Location loc2, EntityType entity, long spawnrate, int areaentitylimit,
			int chunkentitylimit, int slopelimit, Main plugin) {
	...
	void spawn() {
		...
		while (!found && searchcount < 5 && countEntity() < areaentitylimit) {
			...
			if (spawnchunk.getEntities().length < chunkentitylimit && getChunkSlope(spawnchunk) <= slopelimit
					&& spawnchunk.isLoaded()) {
